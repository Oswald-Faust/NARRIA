...
/**
 * Outil « NARR'IA Chat », réutilisable dans la page autonome (/chat) et à l'intérieur
 * d'un projet (/projets/[id]/chat). `basePath` détermine où pointent les navigations
 * (nouvelle conversation, sélection d'une conversation) : on reste ainsi dans le projet.
 */
export function ChatTool({
  ...
  const displayTitle = loadedTitle;
  ...
  async function importFile(file: File) {
    ...
    try {
      ...
      if (!res.ok || !data?.text) {
        setComposerError(data?.error ?? "Impossible d'extraire le contenu du fichier.");
        return;
      }

      const title = data.title || file.name;
      const extracted = [
        `Document importé : ${title}`,
        data.author ? `Auteur : ${data.author}` : "",
        `Fichier source : ${data.sourceFilename || file.name}`,
        "",
        data.text,
      ].filter(Boolean).join("\n");
  ...
  function exportConversation() {
    ...
    const lines = [
      ...
      ...messages.flatMap((message) => [
        `## ${message.role === "user" ? "Utilisateur" : "NARR'IA"}`,
        "",
        messageText(message) || "_Message sans contenu textuel._",
        "",
      ]),
    ];
    ...
    link.href = url;
    link.download = `${slugify(displayTitle || "conversation-narria")}.md`;
...
function slugify(value: string): string {
  ...
  return slug || "conversation-narria";
}
